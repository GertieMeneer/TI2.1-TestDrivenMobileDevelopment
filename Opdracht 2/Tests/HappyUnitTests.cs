namespace Tests
{
    public class HappyUnitTests
    {
        ...
        public async Task TestLoadingLamps()
        {
            ...
            mockMainViewModel.Setup(mock => mock.LoadLamps()).ReturnsAsync("{\"lights\": {" +
        "\"1\": {" +
            "\"name\":\"Cosmic Glow\"," +
            "\"state\": {\"on\":true,\"bri\":75,\"hue\":77,\"sat\":120}," +
            "\"type\":\"SmartLED\"," +
            "\"swversion\":\"v2.0\"," +
            "\"uniqueid\":\"CG-2024-1234\"" +
        "}," +
        "\"2\": {" +
            "\"name\":\"lichtje\"," +
            "\"state\": {\"on\":false,\"bri\":10,\"hue\":2,\"sat\":444}," +
            "\"type\":\"SmartLED\"," +
            "\"swversion\":\"v2.0\"," +
            "\"uniqueid\":\"CG-2314-432\"" +
        "}," +
        "\"3\": {" +
            "\"name\":\"fire\"," +
            "\"state\": {\"on\":true,\"bri\":100,\"hue\":67,\"sat\":33}," +
            "\"type\":\"SmartLED\"," +
            "\"swversion\":\"v2.0\"," +
            "\"uniqueid\":\"CG-5353-423424\"" +
        "}" +
    "}}");
            ...
            mockMainViewModel.Setup(mock => mock.GetUserIdAsync()).ReturnsAsync("3ce1b7fbb5910827a555c50c749c42b");
        ...
        public async Task TestLampFunctionality()
        {
            ...
            Assert.True(lamp.Status == true);

            // Set brightness
            Assert.True(lamp.Brightness == 75);
            await lamp.SetBrightness(60);
            Assert.True(lamp.Brightness == 60);

            // Set color
            Assert.True(lamp.Hue == 77 && lamp.Sat == 120);
            await lamp.SetColor(25, 10);
            Assert.True(lamp.Hue == 25 && lamp.Sat == 10);
        }
    }
}